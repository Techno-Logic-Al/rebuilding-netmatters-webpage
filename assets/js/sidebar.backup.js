(() => {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (isTopNavLink(link)) {
            // Top-level navbar anchors are placeholders: no navigation
            event.preventDefault();
            try { link.blur(); } catch {}
            clearNavFocus();
            return;
        ...
        if (isDropdownLink(link)) {
            ...
            if (href === '#') {
                // Placeholder dropdown item: no navigation, clear focus
                event.preventDefault();
                try { link.blur(); } catch {}
                clearNavFocus();
                return;
            }
            // Real dropdown link: drop focus to avoid lingering :focus-within
            try { link.blur(); } catch {}
            clearNavFocus();
        }
    });})();
